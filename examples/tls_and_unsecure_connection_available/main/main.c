...
static void restart(void *_) {
  for (;;) {
    ...
    if (tls_connection)
    {
      ...
      ESP_LOGI("main", "TLS connection starting!");
      (ret) ? esp_mqtt_start(MQTT_HOST, MQTT_PORT_TLS, "esp-mqtt-tls", MQTT_USER, MQTT_PASS) : 0;
    }
    else
    {
      ESP_LOGI("main", "Unsecure connection starting!");
